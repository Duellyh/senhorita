...
class _VendasViewState extends State<VendasView> {
  ...
  Future<void> carregarDados() async {
    ...
    final docUsuario = await FirebaseFirestore.instance
        ...
        .doc(usuario.uid)
        .get();
    final nomeLogado = docUsuario.data()?['nomeUsuario'];

    final listaFuncionarios = await buscarFuncionarios();

    // Garante que o nome do logado esteja incluído na lista (caso tenha sido filtrado fora)
    if (nomeLogado != null && !listaFuncionarios.contains(nomeLogado)) {
      listaFuncionarios.add(nomeLogado);
    }

    setState(() {
      funcionarioSelecionado = nomeLogado;
  ...
  Future<void> realizarVenda() async {
    ...
    try {
      ...
      final vendaRef = await firestore.collection('vendas').add({
        ...
        'funcionarioId': FirebaseAuth.instance.currentUser?.uid,
        'funcionarioNome':
            funcionarioSelecionado ?? nomeUsuario, // Só para exibir
