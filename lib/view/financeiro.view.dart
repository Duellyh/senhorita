...
pdf.addPage(
  pw.MultiPage(
    ...
    build: (context) {
      return [
        pw.Text('📊 Relatório Financeiro', style: pw.TextStyle(fontSize: 24)),
        pw.SizedBox(height: 16),
        pw.Text('Período: ${DateFormat('dd/MM/yyyy').format(inicioFiltro)} - ${DateFormat('dd/MM/yyyy').format(fimFiltro)}'),
        ...
        pw.Text('Valor Gasto Total: R\$ ${valorGastoTotal.toStringAsFixed(2)}'),
        pw.Text('Lucro: R\$ ${lucro.toStringAsFixed(2)}'),
        pw.SizedBox(height: 12),

        pw.Divider(),

        pw.Text('🧾 Detalhes das Vendas', style: pw.TextStyle(fontSize: 18)),
        pw.SizedBox(height: 8),
        ...
        pw.Text('💳 Formas de Pagamento', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
        pw.SizedBox(height: 8),
        pw.Table.fromTextArray(
        ...
        pw.Text('👤 Vendas por Funcionário', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
        pw.SizedBox(height: 8),
        pw.Table.fromTextArray(
        ...
        pw.Text('📦 Vendas por Categoria', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
        pw.SizedBox(height: 8),
        pw.Table.fromTextArray(
        ...
        pw.Text('📆 Vendas por Dia', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
        pw.SizedBox(height: 8),
        pw.Table.fromTextArray(
          headers: ['Dia', 'Valor'],
  ...
  await Printing.sharePdf(bytes: await pdf.save(), filename: 'relatorio_vendas.pdf');
}

String _getMelhorFuncionario() {
  if (vendasPorFuncionario.isEmpty) return 'Nenhum';
  final entry = vendasPorFuncionario.entries.reduce((a, b) => a.value > b.value ? a : b);
...
Future<void> carregarEstatisticas() async {
  ...
  final agora = DateTime.now();
  final inicioFiltro = dataInicio ?? DateTime(agora.year, agora.month, 1);
  final fimFiltro = (dataFim ?? DateTime.now()).add(const Duration(hours: 23, minutes: 59, seconds: 59));
  ...
  Map<String, double> tempVendasPorCategoria = {};  // Declarar aqui fora, uma única vez
  ...
  for (var doc in snapshot.docs) {
    ...
    tempVendasPorFuncionario[funcionario] = (tempVendasPorFuncionario[funcionario] ?? 0) + valor;
    ...
    for (var item in itens) {
      ...
      tempVendasPorCategoria[categoria] = (tempVendasPorCategoria[categoria] ?? 0) + total;
    }
  }
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                title == "Vendas" ? value.toInt().toString() : "R\$ ${value.toStringAsFixed(2)}",
          ...
          return BarChartGroupData(x: i, barRods: [
            BarChartRodData(
              ...
              backDrawRodData: BackgroundBarChartRodData(
                show: true,
                toY: maxY, // aqui deve ser o maxY para desenhar fundo completo
